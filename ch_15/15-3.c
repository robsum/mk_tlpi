...
#include <sys/stat.h>
#include <time.h>
#include "mk_linux_programming_interface/tlpi-dist/lib/file_perms.h"
...
displayStatInfo(const struct stat *sb)
{
    ...
    printf("Last file access:         %s nanoseconds: %ld\n", ctime(&sb->st_atime), &sb->st_atim.tv_nsec);
    printf("Last file modification:   %s nanoseconds: %ld\n", ctime(&sb->st_mtime), &sb->st_atim.tv_nsec);
    printf("Last status change:       %s nanoseconds: %ld\n", ctime(&sb->st_ctime), &sb->st_atim.tv_nsec);
}
